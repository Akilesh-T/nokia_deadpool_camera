...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latitude, double longitude) {
        ...
        double solarTransitJ2000 = ((((double) (J0 + n)) + arcLongitude) + (0.0053d * Math.sin((double) meanAnomaly))) + (-0.0069d * Math.sin(2.0d * solarLng));
        double solarDec = Math.asin(Math.sin(solarLng) * Math.sin(-0.0f));
        ...
        if (cosHourAngle >= 1.0d) {
            ...
            this.sunset = Math.round((((double) daysSince2000) + solarTransitJ2000) * 8.64E7d) + UTC_2000;
            this.sunrise = Math.round((solarTransitJ2000 - ((double) daysSince2000)) * 8.64E7d) + UTC_2000;
