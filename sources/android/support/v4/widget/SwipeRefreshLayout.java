...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        float adjustedPercent = (((float) Math.max(((double) dragPercent) - 0.4d, Camera2ParamsFragment.TARGET_EV)) * 5.0f) / 3.0f;
        float extraOS = Math.abs(overscrollTop) - this.mTotalDragDistance;
        int slingshotDist = this.mCustomSlingshotDistance > 0 ? this.mCustomSlingshotDistance : this.mUsingCustomStart ? this.mSpinnerOffsetEnd - this.mOriginalOffsetTop : this.mSpinnerOffsetEnd;
        float slingshotDist2 = (float) slingshotDist;
        float tensionSlingshotPercent = Math.max(0.0f, Math.min(extraOS, slingshotDist2 * 2.0f) / slingshotDist2);
        float tensionPercent = ((float) (((double) (tensionSlingshotPercent / 4.0f)) - Math.pow((double) (tensionSlingshotPercent / 4.0f), 2.0d))) * 2.0f;
        int targetY = this.mOriginalOffsetTop + ((int) ((slingshotDist2 * dragPercent) + ((slingshotDist2 * tensionPercent) * 2.0f)));
