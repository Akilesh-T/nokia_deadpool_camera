...
public class XMPSerializerRDF {
    ...
    private void addPadding(int tailLength) throws XMPException, IOException {
        ...
        if (this.options.getExactPacketLength()) {
            minSize = this.outputStream.getBytesWritten() + (this.unicodeSize * tailLength);
        ...
        minSize = this.options.getNewline().length();
        if (this.padding >= minSize) {
            this.padding -= minSize;
            while (this.padding >= 100 + minSize) {
                writeChars(100, ' ');
                writeNewline();
                this.padding -= 100 + minSize;
            }
    ...
    private String serializeAsRDF() throws IOException, XMPException {
        ...
        for (int level = this.options.getBaseIndent(); level > 0; level--) {
            ...
            tailStr = stringBuilder.toString();
        }
        StringBuilder stringBuilder2 = new StringBuilder();
        stringBuilder2.append(tailStr);
        stringBuilder2.append(PACKET_TRAILER);
        tailStr = stringBuilder2.toString();
    ...
    private void serializeCompactRDFSchemas() throws IOException, XMPException {
        ...
        while (it2.hasNext()) {
            serializeCompactRDFElementProps((XMPNode) it2.next(), 3);
        }
        writeIndent(2);
    ...
    private void declareNamespace(String prefix, String namespace, Set usedPrefixes, int indent) throws IOException {
        if (namespace == null) {
            ...
            if (qname.hasPrefix()) {
                ...
                namespace = schemaRegistry.getNamespaceURI(stringBuilder.toString());
        ...
        if (!hasGeneralQualifiers || emitAsRDFValue) {
            if (node.getOptions().isCompositeProperty()) {
                if (node.getOptions().isArray()) {
                    ...
                    it = node.iterateChildren();
                    while (it.hasNext()) {
                        serializePrettyRDFProperty((XMPNode) it.next(), false, i + 2);
                    }
                    ...
                    it = node.iterateChildren();
                    while (it.hasNext()) {
                        serializePrettyRDFProperty((XMPNode) it.next(), false, i + 1);
                    }
                } else {
                    write(" rdf:parseType=\"Resource\"/>");
                    writeNewline();
            ...
            it = node.iterateQualifier();
            while (it.hasNext()) {
                XMPNode qualifier2 = (XMPNode) it.next();
                if (!RDF_ATTR_QUALIFIER.contains(qualifier2.getName())) {
                    serializePrettyRDFProperty(qualifier2, false, i + 1);
                }
            }
        }
        if (emitEndTag) {
