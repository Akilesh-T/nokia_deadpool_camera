...
class XMPNode implements Comparable {
    ...
    public void addQualifier(XMPNode qualNode) throws XMPException {
        ...
        if (qualNode.isLanguageNode()) {
            ...
            getQualifier().add(this.options.getHasLanguage(), qualNode);
        } else {
    ...
    private void dumpNode(StringBuffer result, boolean recursive, int indent, int index) {
        ...
        if (recursive && hasQualifier()) {
            quals = (XMPNode[]) getQualifier().toArray(new XMPNode[getQualifierLength()]);
            int i3 = 0;
            while (quals.length > i3 && (XMPConst.XML_LANG.equals(quals[i3].getName()) || XMPConst.RDF_TYPE.equals(quals[i3].getName()))) {
                i3++;
            }
            Arrays.sort(quals, i3, quals.length);
            for (i3 = 0; i3 < quals.length; i3++) {
                quals[i3].dumpNode(result, recursive, indent + 2, i3 + 1);
            }
        }
        if (recursive && hasChildren()) {
            quals = (XMPNode[]) getChildren().toArray(new XMPNode[getChildrenLength()]);
            if (!getOptions().isArray()) {
                Arrays.sort(quals);
            }
            while (i < quals.length) {
                quals[i].dumpNode(result, recursive, indent + 1, i + 1);
                i++;
            }
        }
    }
    ...
    /* Access modifiers changed, original: protected */
    public void setParent(XMPNode parent) {
        this.parent = parent;
    }
