...
public final class Longs {
    ...
    public static long fromByteArray(byte[] bytes) {
        Preconditions.checkArgument(bytes.length >= 8, "array too small: %s < %s", Integer.valueOf(bytes.length), Integer.valueOf(8));
    ...
    public static Long tryParse(String string) {
        ...
        while (index2 < string.length()) {
            int index3 = index2 + 1;
            index = string.charAt(index2) - 48;
            if (index < 0 || index > 9 || accum < -922337203685477580L) {
                return null;
            }
            accum *= 10;
            if (accum < Long.MIN_VALUE + ((long) index)) {
                return null;
            }
            accum -= (long) index;
            index2 = index3;
