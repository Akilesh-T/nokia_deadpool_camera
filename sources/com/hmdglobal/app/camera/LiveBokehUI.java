...
import com.hmdglobal.app.camera.LiveBokehModule.AspectRatioDialogCallback;
import com.hmdglobal.app.camera.LiveBokehModule.LocationDialogCallback;
...
public class LiveBokehUI implements PreviewStatusListener, CameraFaceDetectionCallback, PreviewAreaChangedListener, OnAllViewRemovedListener {
    private static final int DOWN_SAMPLE_FACTOR = 4;
    private static final boolean HIDE_BURST_COUNT = true;
    ...
    private final LiveBokehController mController;
    ...
    private final OnTouchListener mPreviewTouchListener = new OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            if (LiveBokehUI.this.mEvoSlider.getVisibility() == 8 || event.getAction() != 2) {
                return false;
            }
            if (LiveBokehUI.this.mZoomBar.getVisibility() == 0) {
                LiveBokehUI.this.mZoomBar.setVisibility(8);
            }
            return LiveBokehUI.this.mEvoSlider.onTouchEvent(event);
        }
    };
    ...
    private class DecodeImageForReview extends DecodeTask {
        ...
        public void onPostExecute(Bitmap bitmap) {
            if (!isCancelled()) {
                LiveBokehUI.this.mIntentReviewImageView.setImageBitmap(bitmap);
                LiveBokehUI.this.mIntentReviewImageView.setBackgroundColor(LiveBokehUI.this.mActivity.getResources().getColor(17170444));
                LiveBokehUI.this.showIntentReviewImageView();
                LiveBokehUI.this.mDecodeTaskForReview = null;
            }
        }
    }
    ...
    private class ZoomChangeListener implements OnZoomChangedListener {
        ...
        /* synthetic */ ZoomChangeListener(LiveBokehUI x0, AnonymousClass1 x1) {
            this();
        }

        public void onZoomValueChanged(float ratio) {
            if (LiveBokehUI.this.mZoomBar != null) {
                LiveBokehUI.this.mZoomBar.zoomRatioChanged(ratio);
            }
            LiveBokehUI.this.mController.onZoomChanged(ratio);
        }
    ...
    public LiveBokehUI(CameraActivity activity, LiveBokehController controller, View parent) {
        ...
        ((View) this.mFocusUI).setOnTouchListener(new OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                if (LiveBokehUI.this.mEvoSlider.getVisibility() == 0) {
                    LiveBokehUI.this.mEvoSlider.dispatchTouchEvent(event);
                }
                return true;
            }
        });
        ...
        this.mEvoSlider.setEvoListener(new EVOChangedListener() {
            public void onEVOChanged(int value) {
                LiveBokehUI.this.mController.onEvoChanged(value);
            }
        });
        ...
        this.mZoomBar.setProgressChangeListener(new ProgressChangeListener() {
            public void onProgressChanged(float ratio) {
                LiveBokehUI.this.mPreviewOverlay.setRatio(ratio);
                LiveBokehUI.this.mController.onZoomChanged(ratio);
            }
        });
    ...
    public void showLocationAndAspectRatioDialog(final LocationDialogCallback locationCallback, final AspectRatioDialogCallback aspectRatioDialogCallback) {
        ...
        locationDialogLayout.setLocationTaggingSelectionListener(new LocationTaggingSelectionListener() {
            public void onLocationTaggingSelected(boolean selected) {
                ...
                if (LiveBokehUI.this.showAspectRatioDialogOnThisDevice()) {
                    LiveBokehUI.this.showAspectRatioDialog(aspectRatioDialogCallback, LiveBokehUI.this.mDialog);
                } else if (LiveBokehUI.this.mDialog != null) {
                    LiveBokehUI.this.mDialog.dismiss();
                }
            }
        });
        ...
        aspectRatioDialogLayout.initialize(new AspectRatioChangedListener() {
            public void onAspectRatioChanged(AspectRatio aspectRatio) {
                callback.onAspectRatioSelected(aspectRatio, new Runnable() {
                    public void run() {
                        if (LiveBokehUI.this.mDialog != null) {
                            LiveBokehUI.this.mDialog.dismiss();
                        }
                    }
                });
    ...
    private void initializeFacebeauty() {
        ...
        this.mFaceBeautyOption.setFaceBeautySettingCallBack(new FaceBeautySettingCallBack() {
            public void updateFaceBeautySetting(String key, int value) {
                LiveBokehUI.this.mController.updateFaceBeautySetting(key, value);
            }
        });
    }
    ...
    public void onFaceDetection(final Rect cameraBound, @Nullable final Face[] faces, @Nullable final ExtendedFace[] extendedFaces, CameraProxy camera) {
        if (this.mFaceView != null && cameraBound != null && faces != null) {
            ...
            this.mMainHalder.post(new Runnable() {
                public void run() {
                    LiveBokehUI.this.mFaceView.setBlockDraw(false);
                    LiveBokehUI.this.mFaceView.setOriginalCameraBound(cameraBound);
                    LiveBokehUI.this.mFaceView.setFaces(faces, extendedFaces);
                }
            });
        }
    }
    ...
    public ArrayList<RectF> filterAndAdjustFaces(boolean mirror, int jpgRotation) {
        ...
        for (int i = 0; i < this.mLatestFaces.length; i++) {
            if (this.mLatestFaces[i].getScore() >= 50) {
                RectF rect = new RectF();
                rect.set(this.mLatestFaces[i].getBounds());
                matrix.mapRect(rect);
                rectFs.add(rect);
            }
        }
