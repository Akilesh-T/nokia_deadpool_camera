...
public class GifResourceDecoder implements ResourceDecoder<InputStream, GifDrawable> {
    ...
    private GifDrawableResource decode(byte[] data, int width, int height, GifHeaderParser parser, GifDecoder decoder) {
        ...
        if (header.getNumFrames() <= 0 || header.getStatus() != 0) {
            bArr = data;
            GifDecoder gifDecoder = decoder;
            return null;
        }
        bArr = data;
        Bitmap firstFrame = decodeFirstFrame(decoder, header, bArr);
        if (firstFrame == null) {
            return null;
        }
        return new GifDrawableResource(new GifDrawable(this.context, this.provider, this.bitmapPool, UnitTransformation.get(), width, height, header, bArr, firstFrame));
    }
